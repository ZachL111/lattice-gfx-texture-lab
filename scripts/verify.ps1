...
powershell -NoProfile -ExecutionPolicy Bypass -File scripts/check-lattice-gfx-texture-detail.ps1

dart tests/test_domain_review.dart

powershell -NoProfile -ExecutionPolicy Bypass -File scripts/check-lattice-gfx-texture-properties.ps1
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }

powershell -NoProfile -ExecutionPolicy Bypass -File scripts/check-lattice-gfx-texture-golden.ps1
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }

powershell -NoProfile -ExecutionPolicy Bypass -File scripts/check-lattice_gfx_texture_lab-casebook.ps1
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
